...
import { ArrowDownRight, ArrowUpRight, ArrowRight, Building2, CalendarClock, CalendarIcon, Check, MoreHorizontal, Pencil, Plus, Search, Sparkles, Trash2 } from "lucide-react"

import { reassignMerchantToUncategorized, renameMerchant } from "@/app/(dashboard)/settings/merchants/actions"
import {
...
export function MerchantsView({ merchants, accounts, categoryGroups, schedules, suggestions, recurringOnly = false, merchantOnly = false }: {
  ...
  const visibleSchedules = React.useMemo(() => {
    ...
    return schedules.filter((schedule) =>
      ...
      (!query || schedule.merchantName.toLowerCase().includes(query) || schedule.category?.toLowerCase().includes(query))
    )
  }, [schedules, scheduleQuery, scheduleStatus])
  ...
  return (
    <div className="flex min-w-0 w-full flex-col gap-6">
      ...
      <section className="rounded-xl border p-5">
        ...
        {schedules.length ? <>
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="relative w-full sm:max-w-xs"><Search className="pointer-events-none absolute top-1/2 left-2.5 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><Input value={scheduleQuery} onChange={(event) => setScheduleQuery(event.target.value)} placeholder="Search schedules" aria-label="Search recurring schedules" className="pl-8" /></div>
            <Select value={scheduleStatus} onValueChange={(value) => value && setScheduleStatus(value as ScheduleStatusFilter)}><SelectTrigger className="w-full sm:ml-auto sm:w-36" aria-label="Schedule status"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All schedules</SelectItem><SelectItem value="active">Active</SelectItem><SelectItem value="paused">Paused</SelectItem></SelectContent></Select>
          </div>
          {visibleSchedules.length ? <div className="grid gap-3 xl:grid-cols-2">{visibleSchedules.map((schedule) => <div key={schedule.id} className={cn("group rounded-xl border p-4 transition-colors hover:bg-muted/20", schedule.status === "paused" && "bg-muted/30 opacity-75")}><div className="flex items-start gap-3"><div className={cn("flex size-10 shrink-0 items-center justify-center rounded-full", schedule.transactionType === "income" ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600")}>{schedule.transactionType === "income" ? <ArrowDownRight className="size-4" /> : <ArrowUpRight className="size-4" />}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="truncate font-medium">{schedule.merchantName}</span><Badge variant={schedule.status === "active" ? "secondary" : "outline"}>{schedule.status === "active" ? "Active" : "Paused"}</Badge></div><p className={cn("mt-1 text-xl font-semibold tabular-nums", schedule.transactionType === "income" && "text-emerald-600")}>{schedule.transactionType === "income" ? "+" : "−"}{format(schedule.amount)}</p></div><DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon-sm" disabled={actionPending === schedule.id} />}><MoreHorizontal /><span className="sr-only">Actions for {schedule.merchantName}</span></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => editSchedule(schedule)}>Edit schedule</DropdownMenuItem><DropdownMenuItem onClick={() => void run(schedule.id, setRecurringScheduleStatus(schedule.id, schedule.status === "active" ? "paused" : "active"), schedule.status === "active" ? "Schedule paused" : "Schedule resumed")}>{schedule.status === "active" ? "Pause schedule" : "Resume schedule"}</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onClick={() => setDeleteTarget(schedule)}>Delete schedule</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div><div className="mt-4 grid grid-cols-2 gap-3 border-t pt-3 text-xs"><div><p className="text-muted-foreground">Repeats</p><p className="mt-1 font-medium">{cadenceLabels[schedule.cadence]}</p></div><div><p className="text-muted-foreground">{schedule.status === "active" ? "Next date" : "Schedule"}</p><p className="mt-1 font-medium">{schedule.status === "active" ? dateFormatter.format(new Date(`${schedule.nextDate}T12:00:00`)) : "Paused"}</p></div><div><p className="text-muted-foreground">Account</p><p className="mt-1 truncate font-medium">{accountName(schedule.accountId, accounts)}</p></div><div><p className="text-muted-foreground">Category</p><p className="mt-1 truncate font-medium">{schedule.category ?? "Uncategorized"}</p></div></div></div>)}</div> : <div className="rounded-xl border border-dashed px-6 py-10 text-center"><h3 className="font-medium">No matching schedules</h3><p className="mt-1 text-sm text-muted-foreground">Try another search or status filter.</p><Button variant="link" className="mt-2" onClick={() => { setScheduleQuery(""); setScheduleStatus("all") }}>Clear filters</Button></div>}
        </> : <div className="flex flex-col items-center rounded-xl border border-dashed px-6 py-12 text-center"><div className="mb-3 flex size-12 items-center justify-center rounded-full bg-muted"><CalendarClock className="size-5 text-muted-foreground" /></div><h3 className="font-medium">No recurring transactions</h3><p className="mt-1 max-w-sm text-sm text-muted-foreground">Add repeating bills, subscriptions, salary, or other regular activity.</p><Button className="mt-4" size="sm" onClick={() => createForMerchant("")}><Plus />Add your first schedule</Button></div>}
      </section>
      ...
      <AlertDialog open={!!merchantDeleteTarget} onOpenChange={(open) => !open && setMerchantDeleteTarget(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Move transactions to Uncategorized?</AlertDialogTitle><AlertDialogDescription>All {merchantDeleteTarget?.count ?? 0} transaction{merchantDeleteTarget?.count === 1 ? "" : "s"} for &ldquo;{merchantDeleteTarget?.name}&rdquo; will be reassigned to the Uncategorized payee. Amounts, categories, dates, and balances will not change. Any recurring schedules will be preserved.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Keep merchant</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => { if (!merchantDeleteTarget) return; const target = merchantDeleteTarget; setMerchantDeleteTarget(null); void run(`merchant:${target.name}`, reassignMerchantToUncategorized(target.name), "Transactions moved to Uncategorized") }}>Move transactions</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
