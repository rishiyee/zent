  ...
  return <div className="flex flex-col gap-6">
    <Card className="gap-0 py-0">
      ...
      <CardContent className="grid gap-6 p-5 lg:grid-cols-[minmax(280px,0.75fr)_minmax(0,1.5fr)] lg:items-center lg:p-8">
        {rows.length ? <div className="relative mx-auto h-[clamp(220px,32vh,280px)] w-full max-w-[320px]">
...
function BarReport({ rows, format }: { rows: Row[]; format: (n: number) => string }) {
  return <ChartContainer config={{ value: { label: "Amount", color: "var(--primary)" } }} className="h-[clamp(300px,48vh,440px)] w-full"><BarChart data={rows.slice(0, 10)} layout="vertical" margin={{ left: 20, right: 24 }}><CartesianGrid horizontal={false}/><XAxis type="number" tickFormatter={(v) => format(v)} /><YAxis type="category" dataKey="category" width={110} tickLine={false} axisLine={false}/><ChartTooltip content={<ChartTooltipContent formatter={(v) => <span className="font-medium">{format(Number(v))}</span>} />} /><Bar dataKey="value" fill="var(--color-value)" radius={[0, 6, 6, 0]} /></BarChart></ChartContainer>
}
...
function Empty() { return <div className="grid min-h-[clamp(260px,40vh,360px)] place-items-center text-center"><div><BarChart3 className="mx-auto mb-3 size-8 text-muted-foreground"/><p className="font-medium">No report data</p><p className="text-sm text-muted-foreground">Try a wider date range or another account.</p></div></div> }
