...
export default async function MerchantDetailPage({ params }: { params: Promise<{ merchant: string }> }) {
  ...
  return <><header className="flex h-14 shrink-0 items-center gap-2 border-b px-4"><SidebarTrigger className="-ml-1" /><Separator orientation="vertical" className="mr-2 h-4" /><Breadcrumb><BreadcrumbList><BreadcrumbItem><BreadcrumbLink href="/merchants">Merchants</BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{name}</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb><div className="ml-auto flex items-center gap-1"><PrivacyToggle /><ModeToggle /></div></header><main className="flex flex-1 flex-col p-4 md:p-6"><MerchantDetailView name={name} transactions={transactions} accounts={accounts} /></main></>
}
